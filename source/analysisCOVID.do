...
local data SchoolClosure_Final_RR.dta
...
gen yearbirth = substr(fec_nac_alu, 1,4)
...
ylabel(, format(%04.2f)) xlabel(, format(%04.2f))
ytitle("Post-pandemic attendance (November 2021)")
xtitle("Pre-pandemic attendance")
...
foreach v of local varr {
    ...
    *(3) week and comuna fe, quarantine control and COVID controls 
    local controls quarantine caseRate pcr positivity p_comunal_m p_comunal_h vaccines 
...
foreach v of local varr {
    ...
    *(3) week and comuna fe, quarantine control and COVID controls
    local controls quarantine caseRate pcr positivity movilidad2 p_comunal_m p_comunal_h
    eststo `en'_3_d: areg `v' SchoolClose2 SchoolOpen_i `controls' i.w `cond', `opt2'
    local `en'_3_d_N=e(N)
    test _b[SchoolClose2]=_b[SchoolOpen_i]
    local `en'_3_d_p=r(p)
...
gen bE              = p_comunal if year <2020&year>=2016
...
foreach v of local varr {
    ...
    eststo: reg `v' `indvar1' OpenAtt OpenDev OpenEmp  `controls' i.w `cond', `opt2'
}
#delimit ;
esttab est1 est5 est9 est2 est6 est10 est3 est7 est11 est4 est8 est12,
     keep(`indvar1' OpenAtt OpenDev OpenEmp);
#delimit cr
exit
